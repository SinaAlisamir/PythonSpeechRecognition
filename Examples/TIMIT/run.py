...
def main():
    ...
    input_size = timit.get_input_size()

    myModel = ctc_model(save_dir='./models/model_timit_mfcc', run_name='timit_mfcc')
    ...
    myModel.define_paths(timit.inputs_train_path, timit.outputs_train_path, timit.inputs_test_path, timit.outputs_test_path)
    myModel.setup(timit.lexicon, input_size, use_cnn=False, num_hidden=256, num_layers=1, mlp_hiddens=[64], learning_rate=1e-3, max_len=800)
    myModel.train_model(num_epochs=450, num_batches=num_batches, num_batches_test=num_batches, keep_prob=0.8, data_usage=1, continue_model=False)

    myModel.restore_model(epoch=36)
