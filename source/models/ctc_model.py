...
class ctc_model():
    ...
    def setup_model(self, use_cnn=False, num_hidden=128, num_layers=1):
        ...
        # outputs, _ = tf.nn.bidirectional_dynamic_rnn(tf.nn.rnn_cell.LSTMCell(self.num_hidden, state_is_tuple=True), tf.nn.rnn_cell.LSTMCell(self.num_hidden, state_is_tuple=True), inputs, self.model_seqlen, dtype=tf.float32)
        ...
        self.logits = logits
        self.ctc_probs = tf.reshape(tf.nn.softmax(logits), [batch_s, -1, self.num_classes])
